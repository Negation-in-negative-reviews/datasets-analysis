...
#     })
FIG_DPI = 100
sns.set(font_scale=1.5, style="white", rc={
    "lines.linewidth": 3,
    "lines.markersize":20,
    "ps.useafm": True,
    "font.sans-serif": ["Helvetica"],
...
def draw_grouped_barplot(data, x_label: str, y_label: str, seaborn_hue: str, bargraph_savepath: str, 
    ...
    lg=plt.legend(handles=[pos_rev_patch, neg_rev_patch], loc='upper left', 
...
#     print(data_df[data_df["text_sentiment"]=="negative"])
    ...
    if negation:
        pos_sent_patch = mpl.patches.Patch(color=colors[0], label='Negation before positive lexicon')
        neg_sent_patch = mpl.patches.Patch(color=colors[1], label='Negation before negative lexicon')
    else:
...
def draw_grouped_barplot_four_subbars_liwc(data, colors, x_label: str, y_label: str, 
    ...
    pos_rev_patch = mpl.patches.Patch(facecolor=(1,1,1), alpha=1, edgecolor='black', hatch='', label='Pos reviews')
    neg_rev_patch = mpl.patches.Patch(facecolor=(1,1,1), alpha=1, edgecolor='black', hatch='/', label='Neg reviews')
