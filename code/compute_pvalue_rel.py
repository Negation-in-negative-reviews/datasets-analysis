...
def read_pickle_file(filename):
    ...
    for a_type in analysis_types:
        ...
        for name in dataset_names:
            for review_cat in review_categories:                
                df_name = df[df["name"]==name]
                df_sent = df_name[df_name[review_key]==review_cat]
                # df_sent = df_name
                pos_df_counts = df_sent[df_sent[sentiment_key] == "positive"]["all_samples_data"].to_list()
                neg_df_counts = df_sent[df_sent[sentiment_key] == "negative"]["all_samples_data"].to_list()                
                min_len = min(len(pos_df_counts[0]), len(neg_df_counts[0]))
                statistic, pvalue = stats.ttest_rel(pos_df_counts[0][:min_len],neg_df_counts[0][:min_len])
                stats_data = stats_data.append({
                    "name": name,
                    "analysis_type": a_type,
                    "statistic": statistic,
                    "pvalue": pvalue,
                    "liwc_category": review_cat
                }, ignore_index=True)
